 ...
 */
// create GET HTTP request
$app->get('/api/art/{id:[0-9]+}', function( Request $request, Response $response){
    ...
    try {
      ...
      // query
      $stmt = $db->query( $sql );
      $art["records"] = $stmt->fetchAll( PDO::FETCH_OBJ );
      $db = null; // clear db object
...
$app->get('/api/art/type', function( Request $request, Response $response){
  $id = $request->getAttribute('type');
